...
The first frozen version is `0.2.0`. The `docs/` tree that produced it is unchanged, so the "Next"
version and `0.2.0` are identical until the next cycle adds to `docs/`.
