...
| [screenplay.md](screenplay.md) | `fountain_kit` and what it guarantees, the readers turning a foreign screenplay into Fountain, source provenance, the statistics, the styled editor's document model, scene numbers, the title page, the spell-checking, the editor docks and the syntax guide | `packages/fountain_kit/`, `packages/script_import_kit/`, `packages/spell_kit/`, `lib/ui/pages/editor/` |
| [exports.md](exports.md) | The export panel every mode reaches its documents from, `OcptExportManager` and its sixteen services, the door a foreign screenplay comes in through, the scenario coverage PDF | `lib/managers/export/`, `lib/ui/pages/workspace/widgets/` (the export dialog) |
...
| [budget.md](budget.md) | The money rule, the CNC nomenclature, the quote, the cash journal and its commitments, four views in all — with the financing, the catering and travel, and the sharing still ahead | `lib/ui/pages/workspace/modes/budget/`, `lib/utils/ocpt_budget_*.dart` |
