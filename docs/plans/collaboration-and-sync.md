...
pruning below it, a SQLite store, and a Dockerfile. If versions have shipped, a restore publishes
itself through the snapshot route rather than as a changeset (§3.4). `OcptRelayRemoteStorage` on
the client, plus the pairing screen and the status indicator described in §3.5.
