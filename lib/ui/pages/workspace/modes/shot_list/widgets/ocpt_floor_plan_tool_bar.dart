...
///
/// Offers only the **sequence-scoped** tools this milestone builds: [OcptFloorPlanTool.select],
/// [OcptFloorPlanTool.setElement], and the underlay import action — never a toggled tool, since
/// picking a file is a one-shot action, not a canvas gesture mode. Camera/character/light/arrow/
/// label tools are M6. The zoom cluster on the trailing edge reads/writes
/// [viewportController] directly (a discrete click is already a "settled" zoom, unlike the
/// canvas's own scroll-wheel zoom — see that controller's own doc comment) and stays available
/// under [isReadOnly], since zoom only reads.
class OcptFloorPlanToolBar extends StatelessWidget {
  ...
  Widget _buildToolButton(
    ...
    final isWithheld = isReadOnly && tool != OcptFloorPlanTool.select;
