...
/// A dialog letting the user pick the one-off options a *Day Out of Days* export runs with: the
/// page format, the title page, and which days the table crosses.
///
/// Modelled on `OcptScheduleShootingPlanExportDialog`, whose page-format dropdown, title-page
...
/// for a whole-shoot document is printing the whole shoot, and the days ticked are exactly the range
/// every `SW`/`WF` in the table is read against.
///
/// Use [show] to display it and get back the resulting [OcptDayOutOfDaysExportOptions], or null if
/// the user cancelled.
class OcptScheduleDayOutOfDaysExportDialog extends StatefulWidget {
  ...
  /// Class constructor
  const OcptScheduleDayOutOfDaysExportDialog({required this.current, required this.days, super.key});

  /// Shows the dialog and returns the [OcptDayOutOfDaysExportOptions] the user applied, or null if
  /// they cancelled it.
  static Future<OcptDayOutOfDaysExportOptions?> show(
    BuildContext context, {
    required OcptPageSetup current,
    required List<OcptShootingDay> days,
  }) => showDialog<OcptDayOutOfDaysExportOptions>(
    context: context,
    builder: (context) => OcptScheduleDayOutOfDaysExportDialog(current: current, days: days),
  );

  @override
  State<OcptScheduleDayOutOfDaysExportDialog> createState() =>
      _OcptScheduleDayOutOfDaysExportDialogState();
}

/// The state of [OcptScheduleDayOutOfDaysExportDialog].
class _OcptScheduleDayOutOfDaysExportDialogState
    extends State<OcptScheduleDayOutOfDaysExportDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      title: Text(tr.scheduleExportDayOutOfDaysDialogTitle),
  ...
  /// Pops the dialog returning the resulting [OcptDayOutOfDaysExportOptions], the selected days in
  /// [OcptScheduleDayOutOfDaysExportDialog.days]' own order rather than selection order — that order
  /// is what the table's own first and last convoked days are read against.
  ///
  ...
  /// [OcptScheduleDayOutOfDaysExportDialog.show] caller. Like the shooting plan's own dialog and
  /// unlike the two call sheet ones, it never disables its own export button while no day is ticked:
  /// an empty selection still produces a readable document, which `OcptDayOutOfDaysPdfService`
  /// already handles as a one-note page.
  void _submit() {
    final options = OcptDayOutOfDaysExportOptions(
    ...
    globalGetIt().get<OcptRouterManager>().pop<OcptDayOutOfDaysExportOptions>(options);
  }
}
