...
/// The options a one-off one-line schedule export runs with: the physical page [format], its
/// [margins], which days are printed, and whether the title page is printed.
///
...
class OcptOneLineScheduleExportOptions extends Equatable {
  /// The physical page format to typeset the exported document with. The table itself is always
  /// printed landscape regardless (`OcptOneLineSchedulePdfService`'s own doc comment); this is the
  ...
  /// The ids of the `OcptShootingDay`s to print, in the order they are printed.
  ...
  /// Class constructor
  const OcptOneLineScheduleExportOptions({
  ...
  @override
  String toString() =>
      "OcptOneLineScheduleExportOptions(format: $format, margins: $margins, dayIds: $dayIds, "
