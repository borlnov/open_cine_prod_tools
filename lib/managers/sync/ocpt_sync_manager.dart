...
/// over.
///
/// This step of that plan lands [changesetService]'s **outbound** half — generating a changeset of
/// a replica's own un-pushed local edits and appending it to a relay — plus the [OcptRemoteStorage]
/// seam and [OcptFolderRemoteStorage], the directory transport that exercises it with no network at
/// all and stays afterwards as the desktop fallback
/// (`docs/adr/0009-offline-first-sync-through-a-domain-blind-relay.md`). Applying an incoming
/// changeset, the per-column merge and the screenplay's own three-way merge follow in later steps —
/// this manager depends on [OcptPropertiesManager] (the device id a changeset is stamped with) and
/// [OcptProjectsManager] (the open project a changeset is generated from and applied to) for exactly
/// that reason, ahead of the code that reads either one.
class OcptSyncManager extends AbsWithLifeCycle {
  /// The service turning a replica's own un-pushed local edits into a changeset and appending it to
  /// a relay.
