...
void main() {
  ...
  /// Pumps [OcptEditorExportPdfOptionsDialog] directly (no [showDialog]/`.show`), pre-filled with
  ...
  Future<void> pumpDialog(
    ...
    await tester.pumpWidget(
      _wrapWithLocalization(OcptEditorExportPdfOptionsDialog(current: current)),
    );
  ...
  testWidgets("tapping Export pops the router manager with the format and both checkboxes on", (
    ...
    final popped = routerManager.poppedValue! as OcptPdfExportOptions;
  ...
  testWidgets("unchecking both checkboxes carries them off into the popped options", (
    tester,
  ) async {
    await pumpDialog(tester);
    final context = tester.element(find.byType(OcptEditorExportPdfOptionsDialog));
